...
import com.example.traccite.models.Admin;
...
public class MainActivity extends AppCompatActivity {
  ...
  private void fnAttemptLogin(String email, String pwd) {
    final String emailEntered = email;
    AppTraCCite.getFirebaseAuth().signInWithEmailAndPassword(email, pwd).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
      @Override
      public void onComplete(@NonNull Task<AuthResult> task) {
        if (task.isSuccessful()) {
          Toast.makeText(getApplicationContext(), "Success Login", Toast.LENGTH_LONG).show();
          if (emailEntered.equals("jytan@ourkidslearningjourney.org")) {
            Intent mainIntent = new Intent().setClass(
              MainActivity.this,
              Admin.class);

            Bundle mainBundle = ActivityOptionsCompat.makeCustomAnimation(
              getApplicationContext(),
              R.transition.transition_pull_in_fromright,
              R.transition.transition_push_out_toleft).toBundle();

            startActivity(mainIntent, mainBundle);
          }
        } else {
          Toast.makeText(getApplicationContext(), "NOT Success Login", Toast.LENGTH_LONG).show();
        }
      }
    });
  }
